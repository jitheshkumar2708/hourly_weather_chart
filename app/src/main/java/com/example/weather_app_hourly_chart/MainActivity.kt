...
import com.example.weather_app_hourly_chart.R
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        rb.setOnCheckedChangeListener(RadioGroup.OnCheckedChangeListener { group, checkedId ->
            ...
            when (checkedId) {
                toggleDay.id -> weatherTask().execute()
                toggleHourly.id -> dayTask().execute()
            }
        })
        weatherTask().execute()
    ...
    inner class weatherTask : AsyncTask<String, Void, String>() {
        override fun onPreExecute() {
            ...
            val mCharView: LineChart = findViewById<LineChart>(R.id.liner_chart)
            mCharView.setNoDataText("")
            mCharView.setBackgroundColor(Color.YELLOW)
            val xvalue = ArrayList<String>()
            updatedAtText.let { xvalue.add(it) }
            val lineentry = ArrayList<Entry>()
            lineentry.add(Entry(20f, updatedAtText.length))
            val lineDataSet = LineDataSet(lineentry, "Temperature")
            lineDataSet.color = resources.getColor(R.color.colorAccent)
            val data = LineData(xvalue, lineDataSet)
            mCharView.data = data
            mCharView.animateXY(3000, 3000)
        }
    ...
    inner class dayTask : AsyncTask<String, Void, String>() {
        override fun onPreExecute() {
            super.onPreExecute()
            /* Showing the ProgressBar, Making the main design GONE */
            findViewById<ProgressBar>(R.id.loader).visibility = View.VISIBLE
